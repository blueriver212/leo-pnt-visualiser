...
window.onload = async () => {
    ...
    const hasLineOfSight = (pos1, pos2) => {
      ...
      if (!pos1 || !pos2) return false;
      const toSat = Cartesian3.subtract(pos2, pos1, new Cartesian3());
      const toEarth = Cartesian3.negate(pos1, new Cartesian3());
      const angle = Cartesian3.angleBetween(toSat, toEarth);
      return angle > Math.PI / 2;  // more than 90° → visible
    };
  ...
  viewer.clock.onTick.addEventListener((clock) => {
    ...
    // Figure out which sats are in LOS
    const visible = allGnssSats.filter(({ entity }) => {
      const satPos = entity.position.getValue(time);
      return satPos && hasLineOfSight(issPos, satPos);
    });

    // Count by constellation
    let gpsCount = 0, galileoCount = 0;
    for (const { name } of visible) {
      if (name.includes('GPS')) gpsCount++;
      else if (name.toLowerCase().includes('galileo')) galileoCount++;
    }
    ...
    summaryCell.innerHTML = `<strong>Total: ${totalCount} <br> GPS: ${gpsCount} | Galileo: ${galileoCount}`;
